...
class Drone2dEnv(gym.Env):
    ...
    def __init__(
        ...
        randomize_pad_position: bool = False,    # If True, randomize pad X pos each episode
        ...
        lander_height : float = 0.2,              # height in meters
        ...
        reward_pad_contact: float = 5.0,        # Reward for just making contact with the pad
    ):
        """
    ...
    def init_pymunk(self):
        """
        Initializes the Pymunk physics simulation space for a new episode.
        This involves creating the space, setting gravity, defining collision types,
        adding the ground, landing pad, and the drone with its initial state.
        This method is typically called by `reset()`.
        """
        # 1. Initialize the Pymunk Physics Space
        self.space = pymunk.Space()  # Create a new empty physics simulation space
        # Set the global gravity vector (pulling objects downwards along the Y-axis)
        self.space.gravity = Vec2d(0, -self.gravity_mag)

        # 2. Define Collision Type Identifiers
        # Unique integer IDs used to distinguish between different object types during collisions
        self.drone_collision_type = 1
        self.pad_collision_type = 2
        self.ground_collision_type = 3

        # 3. Create the Ground
        # Create a static body for the ground (it doesn't move under simulation)
        ground_body = pymunk.Body(body_type=pymunk.Body.STATIC)
        # Create a line segment shape representing the ground surface
        self.ground_shape = pymunk.Segment(
            ground_body,                          # Attach shape to the static ground body
            (0, self.ground_height),              # Start point (left edge at ground height)
            (self.world_width, self.ground_height), # End point (right edge at ground height)
            0.5,                                  # Thickness of the ground segment (for collision)
        )
        self.ground_shape.friction = 0.8  # Set friction coefficient for the ground surface
        # Assign the ground's collision type ID for collision handling
        self.ground_shape.collision_type = self.ground_collision_type
        # Add the ground body and its shape to the simulation space
        self.space.add(ground_body, self.ground_shape)


        # 4. === Landing Pad Setup ===
        # Determine the body type for the pad:
        # KINEMATIC: Can be moved programmatically (velocity changes), but not affected by physics forces (like gravity or collisions). Used for moving platforms.
        # STATIC: Cannot move at all. Used for fixed platforms.
        pad_body_type = (
            pymunk.Body.KINEMATIC if self.moving_platform else pymunk.Body.STATIC
        )
        # Create the physics body for the landing pad with the determined type
        self.pad_body = pymunk.Body(body_type=pad_body_type)

        # --- Set Initial Pad Position (X can be randomized) ---
        # The Y position (vertical) of the pad's base is fixed at the specified target height
        pad_start_y = self.landing_target_y

        # Check if the pad's horizontal position should be randomized for this episode
        if self.randomize_pad_position:
            # Calculate the valid range for the pad's *center* X coordinate
            # Ensure the pad's edges stay within the world boundaries
            min_pad_x = self.landing_pad_width / 2.0 # Minimum center X (left edge at world edge 0)
            max_pad_x = self.world_width - (self.landing_pad_width / 2.0) # Maximum center X (right edge at world edge width)

            # Handle edge case where the world might be too narrow for the pad
            if min_pad_x >= max_pad_x:
                # If range is invalid or zero, default to the center
                pad_start_x = self.world_width / 2.0
                print(f"Warning: World width too narrow ({self.world_width}m) to randomize pad position significantly.")
            else:
                # Choose a random X position uniformly within the valid range
                # Uses self.np_random for reproducibility with seeding
                pad_start_x = self.np_random.uniform(min_pad_x, max_pad_x)
            # Optional print statement for debugging/information
            # print(f"Episode {self.episode_count}: Randomized pad position to X = {pad_start_x:.2f} m")
        else:
            # If not randomizing, use the pre-defined fixed horizontal target position
            pad_start_x = self.initial_landing_target_x

        # Set the calculated starting position (world coordinates) for the pad body
        self.pad_body.position = (pad_start_x, pad_start_y)
        # --- End of Pad Position Setting ---

        # Set the initial velocity of the pad body (always starts stationary, even if moving_platform=True)
        # The velocity for a moving platform will be set in _update_platform_position later.
        self.pad_body.velocity = (0, 0)

        # Define the vertices for the landing pad polygon shape.
        # These coordinates are relative to the pad body's center (position).
        half_w = self.landing_pad_width / 2.0 # Half-width for easier vertex definition
        pad_verts = [
            (-half_w, 0),                       # Bottom-left vertex
            (half_w, 0),                        # Bottom-right vertex
            (half_w, self.landing_pad_height),  # Top-right vertex
            (-half_w, self.landing_pad_height), # Top-left vertex
        ]
        # Create the polygon shape using the defined vertices and attach it to the pad body
        self.landing_pad_shape = pymunk.Poly(self.pad_body, pad_verts)
        self.landing_pad_shape.friction = 1.0 # Set friction coefficient for the pad surface
        # Assign the pad's collision type ID
        self.landing_pad_shape.collision_type = self.pad_collision_type
        # Add the pad body and its shape to the simulation space
        self.space.add(self.pad_body, self.landing_pad_shape)


        # 5. === Drone Initialization ===
        # Define the center of the drone's random starting zone
        center_x = self.world_width / 2.0       # Horizontal center of the world
        center_y = self.world_height * 0.8      # Vertical position, 80% up from the bottom

        # Get the half-range for randomization (radius of the square spawn zone)
        r = self.initial_pos_random_range_m

        # Generate random initial coordinates within the spawn zone [center-r, center+r]
        initial_x = self.np_random.uniform(center_x - r, center_x + r)
        initial_y = self.np_random.uniform(center_y - r, center_y + r)

        # Clip the initial X coordinate to ensure the drone starts fully within the world boundaries
        drone_half_width = self.lander_width / 2.0
        initial_x = np.clip(
            initial_x,                          # Value to clip
            drone_half_width,                   # Minimum allowed X (drone's left edge at world edge 0)
            self.world_width - drone_half_width # Maximum allowed X (drone's right edge at world edge width)
        )

        # Calculate the minimum safe starting Y coordinate to avoid spawning inside ground/pad
        min_start_y = self.ground_height + self.landing_pad_height + self.lander_height

        # Clip the initial Y coordinate to ensure it's above the minimum safe height and below the world top
        initial_y = np.clip(
            initial_y,                          # Value to clip
            min_start_y,                        # Minimum allowed Y
            self.world_height - self.lander_height # Maximum allowed Y (approx, ensures it's within bounds)
        )

        # Generate a small random initial angle (tilt) for the drone in radians
        initial_angle = self.np_random.uniform(-0.1, 0.1) # Small random tilt between approx -5.7 and +5.7 degrees

        # Create the Drone object instance using the calculated initial state
        # The Drone class constructor likely handles creating the drone's body and shapes and adds them to the space.
        self.drone = Drone(
            initial_x,          # Starting X position (meters)
            initial_y,          # Starting Y position (meters)
            initial_angle,      # Starting angle (radians)
            self.lander_height, # Drone height (meters)
            self.lander_width,  # Drone width (meters)
            self.lander_mass,   # Drone mass (kg)
            self.space,         # Pass the Pymunk space for the Drone to add itself to
        )
        # Optional print statement for debugging
        # print(f"Drone starting at: ({initial_x:.2f}, {initial_y:.2f}) m")

        # 6. Configure Drone Shapes and Setup Collision Handlers
        # Iterate through the shapes associated with the drone object
        for shape in self.drone.shapes:
            # Assign the drone's collision type ID to each of its shapes
            shape.collision_type = self.drone_collision_type
            # Set the friction coefficient for the drone's surfaces
            shape.friction = 0.7

        # Set up collision handlers to trigger specific methods when collisions occur
        # Handler for collisions between DRONE and PAD
        handler_pad = self.space.add_collision_handler(
            self.drone_collision_type, self.pad_collision_type
        )
        # Assign the self.collision_begin method to be called when contact starts
        handler_pad.begin = self.collision_begin
        # Assign the self.collision_separate method when contact ends
        handler_pad.separate = self.collision_separate

        # Handler for collisions between DRONE and GROUND
        handler_ground = self.space.add_collision_handler(
            self.drone_collision_type, self.ground_collision_type
        )
        # Assign the same methods (they likely check which object type was hit)
        handler_ground.begin = self.collision_begin
        handler_ground.separate = self.collision_separate

        # 7. Initialize Collision State Variable
        # Counter for the number of active contact points the drone has (used in collision_begin/separate)
    ...
    def collision_begin(self, arbiter, space, data):
        ...
        if is_pad_collision:
            # --- ADDED ---
            self.just_collided_with_pad = True # Signal that contact happened
            # -------------
            if (
    ...
    def _calculate_reward(self, observation):
        ...
        # --- Shaping based on distance, velocity, angle (as before) ---
        ...
        reward += 2.0 * np.exp(-0.1 * dist_to_target) # Example proximity reward
        # Penalties for velocity, angle, angular velocity
        ...
        # --- ADDED: Reward for Pad Contact ---
        if self.just_collided_with_pad:
            reward += self.reward_pad_contact
            self.just_collided_with_pad = False # Reset flag after applying reward for this step
        # --- END ADDED ---

        # --- Terminal State Rewards/Penalties (as before) ---
        if self.landed_safely:
            # Bonus for stable landing
            drone_body = self.drone.body if self.drone else None
            if (
                drone_body
                and drone_body.velocity.length < 0.1 # Check for stability
                and abs(drone_body.angular_velocity) < 0.1
            ):
                reward += self.reward_landing
            else:
                # Still landed, but maybe bounced or slid a bit
                reward += self.reward_un_landing
        elif self.crashed:
            reward -= 50.0 # Crash penalty
        elif self.lost_control:
            reward -= 50.0 # Lost control penalty
        elif self.Battery_empty and not (self.landed_safely or self.crashed):
             # Penalty only if battery runs out mid-air (not after landing/crash)
            reward -= 50.0
        elif self.out_of_bounds:
            reward -= 50.0 # Out of bounds penalty
        # Timeout penalty (applied if none of the above terminal conditions met by max_steps)
